...
export const store = {
  ...
  createNote: (noteData: Partial<Note> & { custom_category_name?: string }, sellerId: string) => {
    ...
    const newNote: Note = {
      ...
      storage_key: noteData.storage_key || `key-${Date.now()}`,
      original_filename: noteData.original_filename || 'document.pdf',
      mime_type: noteData.mime_type || 'application/pdf',
      uploaded_at: noteData.uploaded_at || new Date().toISOString(),
